...
//	UE_LOG(LogABGameSingleton, Error, TEXT("HowManyEntity: %d"), HowManyEntity);
	...
	if (SkillDataTableRef.Succeeded())
	{
		...
		for (FSkillData* Row : Rows)
		{
			...
			SkillDataMap.Add(Row->SkillNameID, *Row);
		}
	}
...
//	UE_LOG(LogABGameSingleton, Error, TEXT("HowManySkill: %d"), HowManySkill);
	...
	if (SkillEffectDataTableRef.Succeeded())
	{
		...
		DataTable->GetAllRows<FSkillEffectData>(TEXT("Load SkillEffectData"), Rows);
...
UABGameSingleton& UABGameSingleton::Get()
{
	...
	{
		UE_LOG(LogABGameSingleton,Error,TEXT("GameSingleton is null."));
		ensureMsgf(false,TEXT("GameSingleton not initialized!"));
...
{
	UE_LOG(LogABGameSingleton, Warning, TEXT("Searching for SkillEffectData for SkillNameID: %s"), *SkillNameID);

	const TArray<FSkillEffectData>* FoundArray = SkillEffectDataMap.Find(SkillNameID);
	if (FoundArray)
	{
		EffectData = *FoundArray;
		return true;
	}
